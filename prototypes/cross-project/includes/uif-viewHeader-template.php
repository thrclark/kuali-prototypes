<!--  proposal development header -->

<header class="container uif-viewHeader-contentWrapper">
    <div id="ueqbqhn" class="uif-viewHeader" data-header_for="LabsProposal">
        <h1 class="uif-headerText">
            <span class="uif-viewHeader-areaTitle">Area Title</span>
            <span class="uif-headerText-span">Doc Title</span> <span class="uif-supportTitle-wrapper">
            <span  class="uif-viewHeader-supportTitle" data-parent="ueqbqhn">Doc Subtitle</span>
            </span> </h1>
        <div id="LabsProposal-DocInfo" class="uif-verticalBoxGroup uif-header-rightGroup uif-documentInfo" data-parent="LabsProposal">
            <div  class="uif-messageField uif-boxLayoutVerticalItem clearfix" data-label="Doc Nbr">
                <label class="uif-label" data-label_>Label:</label>
                <p  class="uif-message">Item</p>
            </div>
            <div  class="uif-messageField uif-boxLayoutVerticalItem clearfix" data-label="Status">
                <label class="uif-label">Label:</label>
                <p  class="uif-message">Item</p>
            </div>
            <div id="u1f206ld" class="uif-messageField uif-boxLayoutVerticalItem clearfix" data-label="Status">
                <label class="uif-label" data-label_>Label:</label>
                <p  class="uif-message">Item</p>
            </div>
            <div id="LabsProposal-MoreDocInfo" class="dropdown uif-boxLayoutVerticalItem clearfix"> <a href="#" class="dropdown-toggle" data-toggle="dropdown">Show more...</a>
                <section id="uhlixhs" class="dropdown-menu uif-gridGroup">
                    <h4 class="uif-headerText">Document Info</h4>
                    <table  class="table table-condensed uif-table-fixed" role="presentation">
                        <tbody>
                            <tr>
                                <th scope="row"> <label class="uif-label" data-label_>Label</label></th>
                                <td class="uif-gridLayoutCell"><div  class="uif-messageField" data-label="Doc Nbr">
                                        <p  class="uif-message">Item</p>
                                    </div></td>
                            </tr>
                            <tr>
                                <th scope="row"> <label class="uif-label" data-label_>Label</label></th>
                                <td class="uif-gridLayoutCell"><div  class="uif-messageField" data-label="Doc Nbr">
                                        <p  class="uif-message">Item</p>
                                    </div></td>
                            </tr>
                            <tr>
                                <th scope="row"> <label class="uif-label" data-label_>Label</label></th>
                                <td class="uif-gridLayoutCell"><div  class="uif-messageField" data-label="Doc Nbr">
                                        <p  class="uif-message">Item</p>
                                    </div></td>
                            </tr>
                            <tr>
                                <th scope="row"> <label class="uif-label" data-label_>Label</label></th>
                                <td class="uif-gridLayoutCell"><div  class="uif-messageField" data-label="Doc Nbr">
                                        <p  class="uif-message">Item</p>
                                    </div></td>
                            </tr>
                            <tr>
                                <th scope="row"> <label class="uif-label" data-label_>Label</label></th>
                                <td class="uif-gridLayoutCell"><div  class="uif-messageField" data-label="Doc Nbr">
                                        <p  class="uif-message">Item</p>
                                    </div></td>
                            </tr>
                            <tr>
                                <th scope="row"> <label class="uif-label" data-label_>Label</label></th>
                                <td class="uif-gridLayoutCell"><div  class="uif-messageField" data-label="Doc Nbr">
                                        <p  class="uif-message">Item</p>
                                    </div></td>
                            </tr>
                            <tr>
                                <th scope="row"> <label class="uif-label" data-label_>Label</label></th>
                                <td class="uif-gridLayoutCell"><div  class="uif-messageField" data-label="Doc Nbr">
                                        <p  class="uif-message">Item</p>
                                    </div></td>
                            </tr>
                        </tbody>
                    </table>
                </section>
            </div>
        </div>
    </div>
    <?php include('uif-actionBar.php') ?>
</header>
<!-- // proposal development header -->